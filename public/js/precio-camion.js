...
$(document).ready(function(){
    ...
    function asignarEventoModalCeldas(){
      $('.mostrar-info').click(function(){
        ...
        if (interval<0) {
            $('#pc-modal .modal-title').text('Modificar precio (La oferta expiro hace  '+Math.abs(interval)+' horas)');
            $('#pc-modal .modal-header').css("border-bottom", "1px solid var(--danger)");
            $('#pc-modal .modal-title').css("color", "var(--danger)");
            // console.log(data.fecha_baja);

        }else if(interval==0) {
            // $('#pc-modal .modal-title').text('Modificar precio (Es una nueva oferta)');
            // $('#pc-modal .modal-header').css("border-bottom", "1px solid #565cc0");
            // $('#pc-modal .modal-title').css("color", "#565cc0");
            // console.log(data.fecha_baja);

        }else if(interval<=48) {
            $('#pc-modal .modal-title').text('Modificar precio (Quedan menos de '+interval+' horas)');
            $('#pc-modal .modal-header').css("border-bottom", "1px solid var(--yellow)");
            $('#pc-modal .modal-title').css("color", "var(--yellow)");
            // console.log(data.fecha_baja);

        }else if(interval<=168) {
            $('#pc-modal .modal-title').text('Modificar precio (Quedan menos de '+interval+' horas)');
            $('#pc-modal .modal-header').css("border-bottom", "1px solid var(--success)");
            $('#pc-modal .modal-title').css("color", "var(--success)");
            // console.log(data.fecha_baja);

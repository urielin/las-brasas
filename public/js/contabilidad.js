    ...
    if($.trim(fecha1) != '' && $.trim(fecha2) != '' ){
          ...
          request = $.get('obtener-retiro-detalle',{fecha1:fecha1,fecha2:fecha2 },function(res){
              ...
              $.each(res.depositosDetalle, function(index,value){
                  ...
                  if (value.tipo == "1") {
                    $('#depositoDetalleTabla').append('<tr><td>' + value.folio +'</td><td>RETIROS DIARIOS</td><td>'+ OPER_DESC +"</td><td>"+SUCU_CODIGO+" </td><td>"+ SUCU_NOMBRE +"</td><td> - </td><td> - </td><td>"+ fecha_ingreso +" </td><td>"+ monto +"</td><td>"+ descripcion +"</td><td>"+ cartola_fecha +'</td><td><button type="button" value=""   class="editar-gestion  btn red btn-50 darken-1"> <i class="material-icons dp48">close</i></button></td></tr>');
                  } else {
                    $('#depositoDetalleTabla').append('<tr><td>' + value.folio +'</td><td>OTROS DEPOSITOS</td><td>'+ OPER_DESC+"</td><td>"+SUCU_CODIGO+" </td><td>"+ SUCU_NOMBRE +"</td><td> - </td><td> - </td><td>"+ fecha_ingreso +" </td><td>"+ monto +"</td><td>"+ descripcion +"</td><td>"+ cartola_fecha +'</td><td><button type="button" value=""  class="editar-gestion  btn red btn-50 darken-1"><i class="material-icons dp48">close</i></button></td></tr>');
                  }
                  if (value.monto == null) {
                    montoTotal+=0;
                  } else {
